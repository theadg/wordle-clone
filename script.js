...
getCurrentRow = () =>{
        for (let i = 0; i < 5; i++){
            ...
            if (rows[n].children[i].textContent !== ''){
                rows[n].children[i].classList.add('white');
            }
        }
}

let correctAns = false, entered = false, notIncluded;
const enter = document.querySelector('#enter');
enter.onclick = () => {
    const wordCopy = word;
    userAns = word.join('');
    console.log(userAns,word);
   
    //if incomplete
    if (word.length < 5){
        //add shake effect
            rows[n].classList.add('invalid-word');
        //remove shake after .5s
            setTimeout(function(){
                rows[n].classList.remove('invalid-word');
            }, 500);
        //show not enough letters info
            notEnoughLetters.classList.add('ease-in');
            notEnoughLetters.classList.remove('hidden');
        //REMOVE not enough letters after 1.5s
            setTimeout(function(){
                notEnoughLetters.classList.add('hidden');;
            }, 1500);
    } else if (word.length = 5 && (wordList.includes(userAns) === true)){
        checkWord();
        removeOutline();
    ...
    } else if (!(wordList.includes(userAns))) {
        // alert("Not included"); //CREATE MODAL
        rows[n].classList.add('invalid-word');
            setTimeout(function(){
                rows[n].classList.remove('invalid-word');
            }, 500);
            notInWordList.classList.add('ease-in');
            notInWordList.classList.remove('hidden');
            setTimeout(function(){
                notInWordList.classList.add('hidden');;
            }, 1500);
        console.log(userAns);
        word = Array.from(userAns);
        notIncluded = true;
    }
...
removeOutline = () =>{
    for (let i = 0; i < 5; i++){
        rows[n].children[i].classList.remove('white');
    }
}
x = 0;
checkFirstLetter = () => {
    for (let i = 0; i < letters.length; i++){
        if (letters[i].textContent == typedButtons[x][0]){
            console.log(letters[i]);
            //getting letter
            
            buttons[n].push(letters[i]);

        }
    }

    const first = rows[n].children[0].textContent
    console.log(first); //<---- testing if we have the right child

    if (answer.includes(first) && answer.indexOf(first) === 0){
        rows[n].children[0].classList.add('correct');

        //remove previous color
        buttons[n][0].classList.remove('almost-correct-g');
        setTimeout(function(){
            buttons[n-1][0].classList.add('correct-g');
        }, 1000);

    } else if (answer.includes(first) && answer.indexOf(first) !== 0){
        rows[n].children[0].classList.add('almost-correct');

        setTimeout(function(){
            buttons[n-1][0].classList.add('almost-correct-g');;
        }, 1000);

    } else {
        rows[n].children[0].classList.add('wrong');
        
        setTimeout(function(){
            buttons[n-1][0].classList.add('wrong-g');;
        }, 1000);
    }
};
...
checkSecondLetter = () => {
    for (let i = 0; i < letters.length; i++){
        ...
        if (letters[i].textContent == typedButtons[x][1]){
               //getting letter
               
            buttons[n].push(letters[i]);
        }
    }
    const first = rows[n].children[1].textContent
    console.log(first); //<---- testing if we have the right child

    if (answer.includes(first) && answer.indexOf(first) === 1){
        setTimeout(function() {  //add animation delay
            rows[n-1].children[1].classList.add('correct');
           }, 200);
        
           //remove previous color
           buttons[n][1].classList.remove('almost-correct-g');
           setTimeout(function(){
            buttons[n-1][1].classList.add('correct-g');
        }, 1000);
    } else if (answer.includes(first) && answer.indexOf(first) !== 1){
        setTimeout(function() {  //add animation delay
            rows[n-1].children[1].classList.add('almost-correct');
           }, 200);
        
           setTimeout(function(){
            buttons[n-1][1].classList.add('almost-correct-g');
        }, 1000);
    } else {
        setTimeout(function() {  //add animation delay
            rows[n-1].children[1].classList.add('wrong');
           }, 200);

        setTimeout(function(){
            buttons[n-1][1].classList.add('wrong-g');
        }, 1000);
...
checkThirdLetter = () => {
    for (let i = 0; i < letters.length; i++){
        ...
        if (letters[i].textContent == typedButtons[x][2]){
               //getting letter
               
            buttons[n].push(letters[i]);
        }
    }
    const first = rows[n].children[2].textContent
    console.log(first); //<---- testing if we have the right child

    if (answer.includes(first) && answer.indexOf(first) === 2){
        setTimeout(function() {  //add animation delay
            rows[n-1].children[2].classList.add('correct');
           }, 400);
        
             //remove previous color
             buttons[n][2].classList.remove('almost-correct-g');
           setTimeout(function(){
            buttons[n-1][2].classList.add('correct-g');
        }, 1000);
    } else if (answer.includes(first) && answer.indexOf(first) !== 2){
        setTimeout(function() {  //add animation delay
            rows[n-1].children[2].classList.add('almost-correct');
           }, 400);
        
           setTimeout(function(){
            buttons[n-1][2].classList.add('almost-correct-g');
        }, 1000);
    } else {
        setTimeout(function() {  //add animation delay
            rows[n-1].children[2].classList.add('wrong');
           }, 400);


           setTimeout(function(){
            buttons[n-1][2].classList.add('wrong-g');
        }, 1000);
    }
};
...
checkFourthLetter = () => {
    for (let i = 0; i < letters.length; i++){
        if (letters[i].textContent == typedButtons[x][3]){
               //getting letter
               
            buttons[n].push(letters[i]);
        }
    }
    const first = rows[n].children[3].textContent
    console.log(first); //<---- testing if we have the right child

    if (answer.includes(first) && answer.indexOf(first) === 3){
        setTimeout(function() {  //add animation delay
            rows[n-1].children[3].classList.add('correct');
           }, 600);
           
        
           //remove previous color
           buttons[n][3].classList.remove('almost-correct-g');
           setTimeout(function(){
            buttons[n-1][3].classList.add('correct-g');
        }, 1000);
           
    } else if (answer.includes(first) && answer.indexOf(first) !== 3){
        setTimeout(function() {  //add animation delay
            rows[n-1].children[3].classList.add('almost-correct');
           }, 600);
        
           setTimeout(function(){
            buttons[n-1][3].classList.add('almost-correct-g');
        }, 1000);
    } else {
        setTimeout(function() {  //add animation delay
            rows[n-1].children[3].classList.add('wrong');
           }, 600);


           setTimeout(function(){
            buttons[n-1][3].classList.add('wrong-g');
        }, 1000);
    }
};
...
checkFifthLetter = () => {
    for (let i = 0; i < letters.length; i++){
        ...
        if (letters[i].textContent == typedButtons[x][4]){
               //getting letter
            //    if (n > 0){
            //     buttons[n-1].push(letters[i]);
            // } else {
            buttons[n].push(letters[i]);
        }
    }
    const first = rows[n].children[4].textContent
    console.log(first); //<---- testing if we have the right child

    if (answer.includes(first) && answer.indexOf(first) === 4){
        setTimeout(function() {  //add animation delay
            rows[n-1].children[4].classList.add('correct');
           }, 800);
        
            //remove previous color
            buttons[n][4].classList.remove('almost-correct-g');
           setTimeout(function(){
            buttons[n-1][4].classList.add('correct-g');
        }, 1000);
    } else if (answer.includes(first) && answer.indexOf(first) !== 4){
        setTimeout(function() {  //add animation delay
            rows[n-1].children[4].classList.add('almost-correct');
           }, 800);
        
           setTimeout(function(){
            buttons[n-1][4].classList.add('almost-correct-g');
        }, 1000);
    } else {
        setTimeout(function() {  //add animation delay
            rows[n-1].children[4].classList.add('wrong');
           }, 800);


           setTimeout(function(){
            buttons[n-1][4].classList.add('wrong-g');
        }, 1000);
    }
};
...
// open rickroll
menu.onclick = () => window.open('https://www.youtube.com/watch?v=dQw4w9WgXcQ&ab_channel=RickAstley');


const notInWordList = document.querySelector('#notInWordList');
...
close.onclick = () => {
    modal.classList.toggle('slide-out');
...
info.onclick = () => {
    modalBG.classList.remove('hidden');//check if letter
...
fillWord(); // <--- testing if working

  const winModal = document.querySelector('#winModal');
  const cancel = document.querySelector('#playCancel');
  ...
  cancel.onclick = () => {
   ...
   backspace.setAttribute('disabled', '');
}
   ...
   resetGame = () => {
    ...
    for (let i = 0; i < 6; i++){
        for (let j = 0; j < 5;j++){
            ...
            rows[i].children[j].classList.remove('almost-correct');
            rows[i].children[j].classList.remove('wrong');
        }
        
    }

    lettersArr.forEach((letter) =>{
        letter.classList.remove('correct-g');
        letter.classList.remove('almost-correct-g');
        letter.classList.remove('wrong-g');
    });
    //reset arrays
    buttons = [ [], [], [], [], [], []];
    typedButtons = [[], [], [], [], [], []];
...
// cancel
const playCancelLose = document.querySelector('#playCancelLose');

playCancelLose.onclick = () => {
    modalBG.classList.add('hidden');
...
//play again
const playAgainLose = document.querySelector('#playAgainLose');

playAgainLose.onclick = () => {
    modalBG.classList.add('hidden');
    loseModal.classList.add('hidden');
